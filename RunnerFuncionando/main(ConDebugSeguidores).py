...
def debugFile(n,msj):
    try:
        linea = '\"' + str(n) + '\" , \"' + dorsal + '\"'
        print("Guardando registro n: {} en el archivo debug.csv" .format(n))
        if existe('myposition.txt'):
            coord = recogerPosGPSMovil()
            linea = linea + " , " + '\"' + str(coord[0]) + "\" , "+ '\"' +str(coord[1]) + "\""
        else:
            coord = "\"No hay coordendas\""
            linea = linea + " , " + coord
        msj = msj.split(";")
        linea = linea + ', \"' + msj[5] + '\"\n'

        f = open ('debugSeguidores.csv', 'a')#Modo 'a' Para a�adir no sobre escribir
        f.write(linea)
        f.close()
        print("Escribirmos el fichero \'debugSeguidores\': :")
        print(linea)
        # print("Mostramos fichero:\n")
        # if existe('debug.csv'):
            # print(open('debugSeguidores.csv').read())
        return True
    except Exception as e:
        print("Algo ha fallado en la contadorDebug", e)
        ...
        try:
            ...
            if ide[0] == 'alerta':
                if ide[1] not in alertas:
                    ...
                    for z in ide[1]:
                        if z ==";":
                            ...
                            #print("Cuenta-->",cuenta)
                    if cuenta == 1:
                        print("mensaje personalizado:", ide[1])
                        dor = ide[1].split(';')
                        if dor[0]==dorsal:
                ...
                if ide[0]==dorsal:
                    ...
                    f.write(dor)
                    f.close()
                    if debugFile(contadorDebug,dor):
                        contadorDebug += 1
                        print("Contador Actualizado:", contadorDebug)
...
def enviarPos(dorsal,dorsalDefault):
    ...
    while True:
        try:
            ...
            print("\tPotencia en la última transimision (tx_power)( {} dBm )" .format(lora.tx_power()) )
            j+=1
...
###     Variables
#Formato definir Lora con más parámetros:
...
print("Tiempo {} obtenido entre envio de coordendas" .format(tiempoEntreCoordenadas) )
###     Main
main()
